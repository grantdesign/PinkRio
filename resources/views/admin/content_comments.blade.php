<div class="page-header">
	<div class="row">
		<div class="col-md-6 col-sm-12">
			<div class="title">
			</div>
			<nav aria-label="breadcrumb" role="navigation">
				<ol class="breadcrumb">
					<li class="breadcrumb-item"><a href="{{ route('admin') }}">Главная</a></li>
					<li class="breadcrumb-item active" aria-current="page">{{ $title }}</li>
				</ol>
			</nav>
		</div>
	</div>
</div>

@isset($comments)
<div class="card-box mb-30">
	<h2 class="h4 pd-20">Все комментарии</h2>
	<table class="data-table table nowrap">
		<thead>
			<tr>
				<th class="table-plus datatable-nosort">Имя</th>
				<th>Email</th>
				<th>Сайт</th>
				<th>Текст</th>
				<th class="datatable-nosort">Действие</th>
			</tr>
		</thead>
		<tbody>
			@foreach($comments as $comment)
				<tr>
					<td class="table-plus">
						<h5 class="font-16">{{ $comment->name }}</h5>
					</td>
					<td>
						<p>{{ $comment->email }}</p>
					</td>
					<td>
						<p>{{ $comment->site }}</p>
					</td>
					<td>
						<p>{{ $comment->text }}</p>
					</td>
					<td>
						<div class="dropdown">
							<a class="btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle" href="#" role="button" data-toggle="dropdown">
								<i class="dw dw-more"></i>
							</a>
							<div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
								<a class="dropdown-item" href="{{ route('article', $comment->article_id) }}"><i class="dw dw-eye"></i><button class="btn">Просмотр</button></a>
								<form action="{{ route('comment_edit', $comment->id) }}" method="post">
									@csrf
									@method('DELETE')
									<a class="dropdown-item" href=""><i class="dw dw-delete-3"></i><button type="submit" class="btn">Удаление</button></a>
								</form>
							</div>
						</div>
					</td>
				</tr>
			@endforeach
		</tbody>
	</table>
	<div class="pl-4 pb-4">{{ $comments->links() }}</div>
</div>
@endisset